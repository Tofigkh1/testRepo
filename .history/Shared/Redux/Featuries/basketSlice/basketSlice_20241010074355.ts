...
type BasketPostDataType = {
    ...
    allDescription: string;
    id: string;
    Argument of type 'string' is not assignable to parameter of type 'BasketPostDataType'.ts(2345)
(parameter) product_id: string?: string ;
};
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchBasket.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.error.message ?? null; // Eğer undefined ise null ata
            })
