...
const BasketMenu = () => {
  ...
  const handleDeleteAllBasket = () => {
    const basketId = basket?.data?.id; // Sepetin id'sini buradan alabilirsiniz
    ...
    dispatch(deleteAllBasket(basketId)).then((action) => {
      if (action.type === deleteAllBasket.rejected.type) {
        ...
        dispatch(fetchBasket());  // Sepeti güncelle
        toast({
        ...
        <div ref={menuRef} className={`${styles.menu} ${isMenuOpen ? styles.menuOpen : styles.menuClosed}`}>
          {basketItems.length > 0 ? (
            <div>
              ...
              {basket?.data?.id?.map((items, index) => (
  <button key={index} onClick={() => handleDeleteAllBasket(items.id)}>
    click
  </button>
))}
