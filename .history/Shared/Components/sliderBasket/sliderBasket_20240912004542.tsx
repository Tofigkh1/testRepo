...
const BasketMenu = () => {
  ...
  const basketId = basketItems.length > 0 ? basketItems[0]?.id : null;
  ...
  const handleDeleteAllBasket = () => {
    ...
    dispatch(deleteAllBasket(basketId)).then((action) => {
      if (action.type === deleteAllBasket.rejected.type) {
        ...
        dispatch(fetchBasket()); // Refresh the basket
        toast({
        ...
        <div ref={menuRef} className={`${styles.menu} ${isMenuOpen ? styles.menuOpen : styles.menuClosed}`}>
          {basketItems.length > 0 ? (
            <div>
              ...
              <button onClick={handleDeleteAllBasket} className="bg-red-500 p-2 rounded-full">
        Clear All Basket
      </button>
