...
const BasketMenu = () => {
  ...
  const [initialBasketId, setInitialBasketId] = useState<string | null>(null);

  useEffect(() => {
    if (!initialBasketId && basketId) {
      setInitialBasketId(basketId); // İlk gelen basketId'yi kaydet
    }
  }, [basketId]);
  
  useEffect(() => {
    if (!initialBasketId) return;
  
    const startTimeKey = `basketTimerStartTime_${initialBasketId}`;
    ...
    const intervalId = setInterval(() => {
      ...
      if (newProgress >= 100) {
        ...
        handleDeleteAllBasket(initialBasketId); 
