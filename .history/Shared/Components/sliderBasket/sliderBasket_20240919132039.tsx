...
const BasketMenu = () => {
  ...
  useEffect(() => {
    // Eğer basketId yoksa interval başlatma
    if (!basketId) return;
    ...
    const intervalId = setInterval(() => {
      ...
      if (newProgress >= 100) {
        ...
        handleDeleteAllBasket(basketId); // Sepeti temizle
        localStorage.removeItem(startTimeKey); // LocalStorage'daki zamanı temizle
      } else {
        setProgress(newProgress);
        setRemainingTime(duration - elapsedTime); // Kalan zamanı güncelle
      }
    }, 100);
  
    // Bu `useEffect` hook'u her basketId değiştiğinde tetiklenir
    // Önceki interval'i temizlemek için return kısmına temizleme fonksiyonu ekliyoruz
