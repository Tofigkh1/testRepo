// src/Components/sliderBasket/sliderBasket.tsx
...
import { fetchBasket, deleteFromBasket, addToBasket, deleteAllBasket } from '../../Redux/Features/basketSlice/basketSlice';
...
const BasketMenu = () => {
  ...
  const handleAddFromBasket = (productId: string) => {
    if (!user?.id) {
      toast({
        title: "Kullanıcı oturumu açık değil.",
        status: 'error',
        duration: 2000,
        isClosable: true,
        position: 'top-right',
    ...
    const basketProduct: AddToBasketPayload = {
      user_id: user.id,
  ...
  const handleDeleteFromBasket = (productId: string) => {
    ...
    const basketProduct: DeleteFromBasketPayload = {
      user_id: user.id,
      product_id: productId,
      ageSize: isRectVisible ? "1" : isRectVisible2 ? "2" : null,
    };
    ...
    dispatch(deleteFromBasket(basketProduct)).then((action) => {
      if (deleteFromBasket.rejected.match(action)) {
        ...
        dispatch(fetchBasket());
        toast({
          title: "Ürün sepetten başarıyla kaldırıldı!",
          status: 'success',
          duration: 2000,
          isClosable: true,
          position: 'top-right',
  ...
  const handleDeleteAllBasket = (basketId: string) => {
    ...
    const basketAll: DeleteAllBasketPayload = {
      user_id: user.id,
      basket_id: basketId,
    };
                  ...
                  <div key={index} className='flex flex-col justify-around'>
                    <div className='flex justify-around mt-10'>
                      ...
                      <div className=' flex flex-col'>
                        ...
                        <div className=' mt-5'>
                          <button onClick={() => handleDeleteFromBasket(String(items.product_id))} className="bg-red-500 p-2 rounded-full">
                            <Image src={minus} alt="Delete product" width={35} height={35} />
                          </button>
                        </div>

                        <div className=' mt-5'>
                          <button onClick={() => handleAddFromBasket(String(items.product_id))} className="bg-red-500 p-2 rounded-full">
