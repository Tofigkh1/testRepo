...
const BasketMenu = () => {
  ...
  const [timer, setTimer] = useState(60);  // 60 saniyelik geri sayım
const [isCountdownActive, setIsCountdownActive] = useState(false);
  ...
  useEffect(() => {
    if (isCountdownActive && timer > 0) {
      ...
      return () => clearInterval(countdown); // Temizleme işlemi
    } else if (timer === 0) {
      // Timer sıfıra ulaştığında ürünü sepetten sil
      handleDeleteFromBasket(basketid);
      setIsCountdownActive(false);  // Geri sayımı durdur
    }
  ...
  const handleAddFromBasket = (productId: string) => {
    ...
    dispatch(addToBasket(basketProduct)).then((action) => {
      if (action.type === deleteFromBasket.rejected.type) {
        ...
        setIsCountdownActive(true);  // Geri sayımı başlat
        setTimer(60);  // Geri sayımı 60 saniyeye sıfırla
      }
    });
  };
