...
const BasketMenu = () => {
  ...
  const handleAddFromBasket = (productId: string) => {
    const basketProduct: BasketPostDataType = {
      ...
      allDescription: "All details about the product", 
      img_url: "imageUrl",    
      price: 100,             
      name: "Product name",   
      count: 1,               // Burada count'u default bir değer ile tanımlıyoruz
      total_count: basket?.data?.total_count || 0,  // Eğer undefined ise 0 kullan
      total_item: basket?.data?.total_item || 0,    
      total_amount: basket?.data?.total_amount || 0 
    };
