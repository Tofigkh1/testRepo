...
const BasketMenu = () => {
  ...
  const duration = 60 * 1000; // 1 dakika süre (ms cinsinden)
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (newProgress >= 100) {
        ...
        handleDeleteAllBasket(basketId); // Zaman dolduğunda tüm ürünleri sepetten kaldır
        localStorage.removeItem(startTimeKey); // Timer sıfırlandığı için kaydı temizle
      } else {
  ...
  const handleDeleteAllBasket = (basketId: string) => {
    ...
    dispatch(deleteAllBasket(basketAll)).then((action) => {
      if (action.type === deleteAllBasket.rejected.type) {
        ...
        const startTimeKey = `basketTimerStartTime_${basketId}`;
        localStorage.removeItem(startTimeKey); // Sepet boşaltıldığında timer'ı sıfırla
      }
    });
