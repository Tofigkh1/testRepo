...
const BasketMenu = () => {
  ...
  const handleClearBasket = () => {
    const basketId = basketItems[0]?.id; // Assuming the first item's basket ID represents the whole basket
    const basketId = {
      user_id: user?.id,
      product_id: productId,
      ageSize: isRectVisible ? "1" : isRectVisible2 ? "2" : null,
    ...
    dispatch(deleteAllBasket(basketId)).then((action) => {
      if (action.type === deleteAllBasket.rejected.type) {
        ...
        dispatch(fetchBasket()); // Refresh the basket data
        toast({
...
{basketItems.length > 0 && (
      <div className='flex justify-end mt-4 mr-5'>
        <button
          ...
          className='bg-red-500 w-64 h-12 rounded-lg font-semibold text-xl'
        >
          Clear Basket
        </button>
      </div>
