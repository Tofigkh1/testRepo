...
const BasketMenu = () => {
  ...
  const basketAllId = basket?.data || []
  console.log("basket",basketAllId);
  ...
  const handleDeleteAllBasket = () => {
    const basketIds = basketItems.map((item) => item.id);  // Basket'teki tüm id'leri topluyoruz

    // Tüm id'lere göre silme işlemi
    basketIds.forEach((basketId) => {
        dispatch(deleteFromBasket(basketId)).then((action) => {
            if (action.type === deleteFromBasket.rejected.type) {
                toast({
                    title: "Failed to remove product from the basket",
                    status: 'error',
                ...
                dispatch(fetchBasket());  // Sepeti güncellemeyi unutmayın
                toast({
                    title: "Product removed from the basket successfully!",
                    status: 'success',
                    duration: 2000,
  ...
  return (
    ...
    <button key={index} onClick={() => handleDeleteFromBasket(item.id)}>
        Sil {item.name}
    </button>
))}

<button onClick={handleDeleteAllBasket}>
    Sepeti Temizle
</button>
