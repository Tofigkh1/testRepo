...
export default function BasketContainer(props: BasketProps) {
  ...
  const basket_list = basketList?.data?.result?.data;
        ...
        {basket_list?.items?.length > 0 ? (
          <>
            <div className="min-h-[550px] flex justify-between flex-col">
              <div>
                <h2 className={styles.user_cabinet_title}>{/* t("Your Basket") */ "Your Basket"}</h2>
                <div className="flex justify-between items-center mb-5">
                  <div className={styles.item_counts}>
                    <BasketSvg />
                    <span>{basket_list?.items.length} {/* t("items") */ "items"}</span>
                  </div>
                  <button
                    onClick={handleClear}
                    className={`lightRed gap-2 flex items-center capitalize ${styles.clear_btn}`}
                  >
                    <RemoveSvg />
                ...
                <div className={styles.basket_list}>
                  {basket_list?.items?.map((product: any) => (
                    <BasketItem
                      total_count={basket_list.total_count}
                      basket_id={basket_list.id}
                      total_amount={basket_list.total_amount}
                      key={product.id}
                      {...product}
                    />
                  ))}
