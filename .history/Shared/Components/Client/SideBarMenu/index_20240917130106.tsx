    ...
    <aside className="h-screen">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all ${
          expanded ? "w-[22%]" : "w-[60px]" // Sidebar expands to 32% width or collapses to 60px
        }`}
      >
        <div className="p-4 pb-2 flex justify-between items-center">
          ...
          <img
            ...
            className={`overflow-hidden transition-all ${
              expanded ? "w-52" : "w-0" // Logo is hidden when sidebar is collapsed
            }`}
            alt="Logo"
          />
...
export function SidebarItem({ icon, text, active, onClick }: SidebarItemProps) {
  ...
  return (
    ...
    >
      ...
      {/* When collapsed, display the text on hover */}
      {!expanded && (
        <span
