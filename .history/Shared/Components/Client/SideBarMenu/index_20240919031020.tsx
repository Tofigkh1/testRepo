...
export default function Sidebar({ children }: { children: ReactNode }) {
  ...
  return (
    <aside className="h-screen mt-1">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 border-spacing-y-96 transition-all ml-3 ${
          expanded ? "w-[140%]" : "w-[95px] " // Sidebar expands to 32% width or collapses to 60px
        }`}
      >
