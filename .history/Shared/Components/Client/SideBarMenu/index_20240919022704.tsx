...
export function SidebarItem({ icon, text, active, onClick }: SidebarItemProps) {
  ...
  return (
    ...
    >
      ...
      {!expanded && (
        <span
          className={` absolute left-full whitespace-nowrap bg-clientButtonGreen text-black rounded-md py-1 px-2 transition-all duration-200 ml-2 opacity-0 group-hover:opacity-100`}
        >
          {text}
        </span>
      )}
