        ...
        <div className="p-4 pb-2 flex justify-between items-center">
         ...
         >Menu</h1>
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-FooterColor hover:bg-mainRed mr-2 w-12 h-10" 
          >
...
export function SidebarItem({ icon, text, active, onClick }: SidebarItemProps) {
  ...
  return (
    <li
      ...
      className={`group relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors  mt-7  ${
        active
    ...
    >
      ...
      {!expanded && (
        <span
          className={` absolute left-full py-2 px-3 my-1 hover:bg-textColorGreen text-gray-600  whitespace-nowrap bg-green-500 text-black rounded-md py-1 px-2 transition-all duration-200 ml-2 opacity-0 group-hover:opacity-100`}
        >
          {text}
        </span>
      )}
