...
export default function Sidebar({ children }: { children: ReactNode }) {
  ...
  return (
    <aside className="h-screen mt-6">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all ml-1 ${
          expanded ? "w-[25%]" : "w-[95px] " // Sidebar expands to 32% width or collapses to 60px
        }`}
      >
        <div className="p-4 pb-2 flex justify-between items-center">
         ...
         >Menu</h1>
          ...
          <button
            ...
            className="p-3 rounded-lg bg-textColorGreen hover:bg-clientButtonGreen mr-2 w-14 h-11 mt-14" 
          >
