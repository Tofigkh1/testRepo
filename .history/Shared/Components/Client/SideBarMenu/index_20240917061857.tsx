...
export function SidebarItem({ icon, text, active, alert }: SidebarItemProps) {
    ...
    return (
      <li
        className={`group relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors text-[35px] ${
          active
            ? "bg-custompurple text-mainRedLight" // Aktif öğe stili
            : "hover:bg-textColorGreen text-gray-600" // Hover stili
        }`}
      >
        {/* İkonun boyutu büyütüldü */}
        <span className="flex items-center justify-center" style={{ width: '35px', height: '35px' }}>
          {React.cloneElement(icon as React.ReactElement, { size: 35 })}
        </span>
  
        {/* Sidebar açıkken text normal siyah renkte olacak ve boyutu 35px olacak */}
        {expanded && (
          <span className="ml-3 text-black transition-all duration-200 text-[35px]">
            {text}
          </span>
        )}
  
        {/* Sidebar kapalıyken hover ile text yeşil arka plan içinde görünecek */}
        {!expanded && (
          <span
            className={`absolute left-full whitespace-nowrap bg-green-300 text-black rounded-md py-1 px-2 transition-all duration-200 ml-2 opacity-0 group-hover:opacity-100 text-[35px]`}
          >
            {text}
          </span>
        )}
