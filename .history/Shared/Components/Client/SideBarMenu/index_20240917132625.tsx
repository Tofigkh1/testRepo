    ...
    <aside className="h-screen">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all ${
          expanded ? "w-[24%]" : "w-[60px]" // Sidebar expands to 32% width or collapses to 60px
        }`}
      >
...
export function SidebarItem({ icon, text, active, onClick }: SidebarItemProps) {
  ...
  return (
    ...
    >
      {/* Icon her iki durumda da büyük kalacak */}
      <span className="text-xl">
        {icon}
      </span>

      {/* Text expanded durumuna bağlı olarak gösterilecek */}
      {expanded && (
