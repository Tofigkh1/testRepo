...
export default function Sidebar({ children }: { children: ReactNode }) {
  ...
  return (
    <aside className="h-screen mt-1   <aside className="h-screen fixed top-0 left-0 z-50">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all ${
          expanded ? "w-[300px]" : "w-[95px]"
        }`} // Adjust the width only on the sidebar itself, not affecting the rest of the layout
      >
        <div className="p-4 pb-2 flex justify-between items-center">
          <h1 className={`overflow-hidden transition-all ${expanded ? "w-32 text-2xl font-semibold" : "w-0"}`}>
            Menu
          </h1>
          <button
            onClick={() => setExpanded((curr) => !curr)}
            className="pl-3.5 rounded-lg bg-textColorGreen hover:bg-clientButtonGreen mr-2 w-14 h-11"
          >
      ...
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all ml-3 ${
          expanded ? "w-[140%] h-[135%]" : "w-[95px]" // Sidebar expands to 32% width or collapses to 60px
        }`}
      >
        <div className="p-4 pb-2 flex justify-between items-center">
