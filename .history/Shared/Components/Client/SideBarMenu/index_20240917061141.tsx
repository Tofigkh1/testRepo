...
export function SidebarItem({ icon, text, active, alert }: SidebarItemProps) {
  const context = useContext(SidebarContext); // Access the sidebar context to check expansion state
    ...
    <li
      className={`relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors ${
        active
          ...
          : "hover:bg-textColorGreen text-gray-600" // Non-active item hover style
      }`}
    >
      {icon} {/* Render the icon */}
      <span
        className={`overflow-hidden transition-all duration-200 ${
          expanded ? "w-auto ml-3 opacity-100" : "w-0 opacity-0" // Text visibility changes based on expansion state
        }`}
      >
      ...
      {alert && (
        <div
          className={`absolute right-2 w-2 h-2 rounded bg-indigo-400 ${
            expanded ? "" : "top-2" // Alert indicator changes position based on expansion state
          }`}
        />
      )}
    </li>
