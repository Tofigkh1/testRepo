...
export function SidebarItem({ icon, text, active, alert }: SidebarItemProps) {
    ...
    return (
      ...
      >
        ...
        {/* Kapalıyken hover ile metin görünecek */}
        <span
          className={`absolute left-full whitespace-nowrap bg-green-300 text-white rounded-md py-1 px-2 transition-all duration-200 ml-2 opacity-0 group-hover:opacity-100 ${
            expanded ? "hidden" : "" // Sidebar açıkken text normal şekilde görünür
          }`}
        >
          {text}
        </span>
