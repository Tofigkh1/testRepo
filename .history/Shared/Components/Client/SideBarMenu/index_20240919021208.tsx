...
export default function Sidebar({ children }: { children: ReactNode }) {
  ...
  return (
    <aside className="h-screen">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all mt-10 ${
...
export function SidebarItem({ icon, text, active, onClick }: SidebarItemProps) {
  ...
  return (
    <li
      ...
      className={`group relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors mt-10  ${
        active
