    ...
    <aside className="h-screen mt-1">
      <nav
        className={`h-full flex flex-col bg-white border-r border-whiteLight3 transition-all ml-3 ${
          expanded ? "w-[24%]" : "w-[95px] " // Sidebar expands to 32% width or collapses to 60px
        }`}
      >
        <div className="p-4 pb-2 flex justify-between items-center">
         ...
         >Menu</h1>
          ...
          >
            {expanded ? <ChevronFirst className=" w-44" /> : <ChevronLast />} {/* Chevron icon changes based on expansion state */}
          </button>
        </div>
