...
const RegisterForm = (props: Props) => {
  ...
  const handleSubmit = (values: RegisterFormValues, { setSubmitting, resetForm }: { setSubmitting: (isSubmitting: boolean) => void, resetForm: () => void }) => {
    (async () => {
      try {
        setLoading(true);
        await postSignUp(values)
          .then(() => {
            ...
            resetForm(); // Formu sıfırlamak için resetForm çağrısı
            setLoading(false);
          })
          .catch((err) => {
            setLoading(false);
  ...
  return (
    <div>
      ...
      <Formik
        ...
        enableReinitialize // Formik'in initialValues'u yeniden başlatmasını sağlar
      >
        {({ isSubmitting }) => (
