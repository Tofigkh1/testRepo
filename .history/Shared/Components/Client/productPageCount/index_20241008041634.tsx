...
const productPageCount = () => {
 ...
 interface BasketPostData {
    ...
    const handleAddFromBasket = (productId: string) => {
        const basketProduct: BasketPostData = {
            user_id: user?.id,                       // Kullanıcı ID'si
            product_id: productId,                   // Ürün ID'si
            ageSize: isRectVisible ? "1" : isRectVisible2 ? "2" : null, // Yaş boyutu
        
            // Zorunlu olan diğer alanlar
            cover_url: items?.img_url || "",         // Ürün resim URL'si (varsayılan boş string)
            amount: items?.price || 0,               // Ürün fiyatı (varsayılan 0)
            created: new Date().toISOString(),       // Ürün eklenme zamanı
            count: items?.count || 1,                // Ürün adedi (varsayılan 1)
            description: items?.description || "",   // Ürün açıklaması (varsayılan boş string)
            rest_id: items?.rest_id || "",           // Restoran/mağaza ID'si (varsayılan boş string)
        
            // Eğer başka zorunlu alanlar varsa, bunları da buraya ekleyebilirsin
        };
