...
<div className={`${styles.scrollContainer}`}>
  {basketItems.map((items, index) => (
    <div key={index} className="flex justify-around items-center mt-10"> {/* items-center: Ortak hizalama */}
      <div className="w-36 h-auto rounded-2xl bg-white flex justify-center ">
        <Image src={items.img_url} alt={`Product Image ${index + 1}`} width={130} height={130} />
      </div>

      <div className="flex-1 ml-6"> {/* flex-1: Esnek genişlik, ml-6: Görseller arasında boşluk */}
        <h1 className="font-bold text-xl text-black">{items.name}</h1>
        <h1>{items.ageSize == 1 ? 30 : 60} Age</h1>

        <div className="flex items-center mt-4"> {/* Flex ayarları düzeltilmiş */}
          <button
