...
const productPageCount = () => {
 ...
 interface BasketPostData {
    ...
    const handleAddFromBasket = (productId: string) => {
        const basketProduct: BasketPostDataType:any = {
            user_id: user?.id,
            product_id: productId,
            ageSize: isRectVisible ? "1" : isRectVisible2 ? "2" : null,
            cover_url: "", // Eksik alanları doldur (örnek olarak boş değer)
            amount: 1,     // Varsayılan değerler ekleyin
            created: Date.now(), // Örneğin, şu anki zaman
            count: 1,      // Default sayısı ekleyin
