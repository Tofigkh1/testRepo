...
const ProductPageCount = () => { // Changed to PascalCase for React component
    ...
    const handleAddFromBasket = (productId: string) => {
        // Find the product in basketItems to get all required properties
        const product = basketItems.find(item => item.product_id === productId);
        if (!product) {
            toast({
                title: "Product not found in the basket.",
                status: 'error',
                duration: 2000,
                isClosable: true,
                position: 'top-right',
        ...
        const basketProduct: BasketPostDataType = {
            ...
            description: product.description, // Ensure this exists
            rest_id: product.rest_id,         // Ensure this exists
            category_id: product.category_id, // Ensure this exists
            allDescription: product.allDescription, // Ensure this exists
    ...
    const handleDeleteFromBasket = (productId: string) => {
        const basketProduct: BasketPostDataType = {
            ...
            product_id: productId,
            ageSize: isRectVisible ? "1" : isRectVisible2 ? "2" : null,
            description: "", // Provide necessary values or handle differently
            rest_id: 0,      // Provide necessary values or handle differently
            category_id: 0,  // Provide necessary values or handle differently
            allDescription: "", // Provide necessary values or handle differently
            // Add other required fields if any
        };
            ...
            {basketItems.map((items, index) => (
                <div key={index} className="flex justify-around items-center mt-10"> {/* items-center: center alignment */}
                    <div className="w-36 h-auto rounded-2xl bg-white flex justify-center items-center">
                        <Image src={items.img_url || '/default-image.png'} alt={`Product Image ${index + 1}`} width={130} height={130} />
                    </div>
                    ...
                    <div className="flex-1 ml-6"> {/* flex-1: flexible width, ml-6: margin between images */}
                        ...
                        <div className="flex items-center mt-4"> {/* Adjusted flex settings */}
                            <button
                                onClick={() => handleDeleteFromBasket(items.product_id)}
                            ...
                            <button
                                onClick={() => handleAddFromBasket(items.product_id)}
