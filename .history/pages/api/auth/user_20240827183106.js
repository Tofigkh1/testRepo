...
export default async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    try {
      ...
      // Optionally fetch the updated user data after claims are set
      const updatedUser = await admin.auth().getUser(decodedToken.userId);
  
      return res.status(200).json({
        message: "Updated user!",
        user: { ...updatedUser, id: decodedToken.userId, ...customClaims },
      });
    } catch (err) {
