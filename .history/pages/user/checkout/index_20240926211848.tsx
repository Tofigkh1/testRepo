...
export default function index() {
  ...
  useEffect(() => {
    ...
    const basketCountt = basketItems.map((item) => {
        if (item.ageSize === 1) {
            ...
            return 0; // Eğer agesize 1 veya 2 değilse 0 döndür.
        }
    });
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false);
    ...
    const handleChange = (e) => {
      const { name, value } = e.target; // Input'tan gelen name ve value'yu alıyoruz
      setUser((prevUser) => ({
        ...prevUser,
        [name]: value, // Hangi input değişiyorsa onu güncelliyoruz
      }));
    };
...
<div>
      ...
      <div className='mt-6 ml-6'>
        <label className='text-grayText2 font-bold'>Adres</label>
        <input
          type="text"
          id="address"
          name="address"
          value={user.address} // State'teki adres değerine bağlı
          onChange={handleChange} // Değer değiştikçe handleChange çalışır
          required
      ...
      <div className='ml-6 mt-6'>
        <label className='text-grayText2 font-bold'>Telefon Numarası</label>
        <input
          type="text"
          id="phoneNumber"
          name="phoneNumber" // name değeri, state'de doğru alanı günceller
          value={user.phoneNumber} // State'teki telefon numarası
          onChange={handleChange} // Değer değiştikçe handleChange çalışır
          required
          className='w-11/12 h-14 p-5 rounded-md'
          placeholder='+994'
        />
               ...
               <div className="flex ml-6 mt-4">
                   ...
                   <h1 className={`ml-2 ${isRectVisiblee2 ? 'text-textColorGreen' : ''}`}>  </h1>
                   ...
                   <h1 className={`ml-2 ${isRectVisiblee ? 'text-textColorGreen' : ''}`}></h1>
               </div>
