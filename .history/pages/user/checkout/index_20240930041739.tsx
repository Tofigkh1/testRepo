...
export default function index() {
  ...
  const basketId = basket?.data?.items;
...
<SimpleForm/>
               ...
               <div className='flex items-center justify-center mt-16'>
                   ...
                   <button
                      className={`w-11/12 h-11 ${(isRectVisible || isRectVisible2) && basketId?.items?.length > 0 && inputVal && inputPhoneNumber && phoneNumRegex && addressValid ? 'bg-textColorGreen' : 'bg-overlayColorGreen'} text-white rounded-sm`}
                      onClick={handleCheckout}
                      disabled={!((isRectVisible || isRectVisible2) && inputVal && inputPhoneNumber && phoneNumRegex && addressValid && basketId?.items?.length  > 0)}
                   >
                      Checkout
                   </button>
           ...
           {/* <div>
            {isRectVisiblee2?<OverlayPayment/>:'' }
           </div> */}
