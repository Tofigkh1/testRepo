...
export default function index() {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false);
    ...
    const handleAddFromBasket = (productId: string) => {
        ...
        dispatchh(addToBasket(basketProduct)).then((action) => {
    ...
    const handleDeleteFromBasket = (productId: string) => {
        ...
        dispatchh(deleteFromBasket(basketProduct)).then((action) => {
            if (action.type === deleteFromBasket.rejected.type) {
                toast({
                    title: "Failed to remove product from the basket",
                    status: 'error',
       ...
       <div className='flex gap-52 justify-between'>
           ...
           <div className=" bg-cardColor w-5/12 mt-6 mr-5 absolute ml-96">
               ...
               <div className="flex ml-6 mt-4">
                   ...
                   <h1 className={`ml-2 ${isRectVisible2 ? 'text-textColorGreen' : ''}`}>dfbxbxx</h1>
                   ...
                   <h1 className={`ml-2 ${isRectVisible ? 'text-textColorGreen' : ''}`}>xfbdxgvzgvz</h1>
               </div>
                   ...
                   {basketItems?.map((items, index) => {
                       ...
                       return (
                           <div className=" flex w-96 " key={index}>
                               <div className=' bg-white w-36 mt-10'>
                                   <Image src={items?.img_url} alt={`Product Image ${index + 1}`} width={130}
                                          height={130}/>
                               </div>

                               <div className=' mt-11 ml-8'>
                                   <h1 className='font-medium text-2xl'>{items.name}</h1>
                                   <div>
                                       <h1>
                                           {items.ageSize == 1 ? 30 : 60}Age
                                       </h1>
                                   </div>
                                   ...
                                   <div className=' mt-5'>
                                       <button key={index} onClick={() => handleDeleteFromBasket(items.id)}
                                   ...
                                   <div className=' mt-5'>
                                       <button key={index} onClick={() => handleAddFromBasket(items.id)}
                                               className="bg-red-500 p-2 rounded-full">
