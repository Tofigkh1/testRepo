          ...
          <div className="flex  justify-center">
            {chooseCategory ? (
              ...
              categories?.map((category) => (
                <div key={category.id} className="w-full h-auto m-4 rounded-2xl border border-whiteLight3">
                <h2 className="text-center text-xl font-bold ">{category.name}</h2>
              
                <div className="flex flex-wrap gap-4">
                  {getProductsByCategory(category.id).map((product: any) => (
                    <div key={product.id} className="w-1/4 md:w-1/5 lg:w-1/6">
                      <ProductCard {...product} onReadMore={() => onDetail(product.id)} />
                    </div>
                  ))}
                </div>
