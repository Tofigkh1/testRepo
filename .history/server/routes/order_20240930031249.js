...
export async function handlerAddOrderPOST(req, res, col) {
  ...
  try {
    ...
    const { fullname, email, delivery_address, payment_method, contact,  } = req.body;

    const userRecrods = await getDataID(ROUTER.RECORDS, records_id);
    ...
    await uptData(ROUTER.RECORDS, basket_id, emptyBasket(decodedToken.userId));
