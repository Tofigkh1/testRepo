...
export async function handlerAddProductCardPOST(req, res, col) {
  ...
  try {
    ...
    // Ürün 1 dakika sonra otomatik olarak silinsin
    setTimeout(async () => {
      try {
        const card = await getQueryData(col, "user_id", decodedToken.userId);
        const userBasket = card?.[0];
        ...
        if (itemIndex != -1) {
          const newItems = userBasket.items.filter(
            (item) => item.id !== product_id
          );
          ...
          await uptData(col, userBasket.id, updatedData);
          console.log(`Product with ID ${product_id} removed after 1 minute.`);
        }
      } catch (error) {
        console.error("Error removing product after 1 minute:", error);
      }
