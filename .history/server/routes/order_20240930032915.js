...
export async function handlerAddOrderPOST(req, res, col) {
  ...
  try {
    ...
    // recrods'tan veri al
    const recordsData = await getQueryData(ROUTER.RECRODS, "userId", decodedToken.userId);
    ...
    // İlk bulunan kayıttan gerekli verileri al
    const record = recordsData[0];
    const { fullname, email, delivery_address, total_amount, items } = record;

    // Body'den yalnızca basket_id alıyoruz
    ...
    const order = new Order(
      ...
      email,
      delivery_address,
      total_amount,  // recrods'tan gelen toplam tutar
      record.contact, // recrods'tan gelen iletişim bilgisi
      record.payment_method, // recrods'tan gelen ödeme yöntemi
      items, // recrods'tan gelen ürünler
      new Date() // Siparişin oluşturulduğu tarih
    );
